...
# file_dir = 'E:\Super-BPD-master\datasets\CoMoFoD\CoMoFoD_seg'
# save_dir = 'E:\Super-BPD-master\datasets\CoMoFoD\CoMoFoD_segimage'
# filenames = os.listdir(file_dir)

# for name in filenames:
#     flux = sio.loadmat(file_dir + '\\' + name)['flux']
# flux = sio.loadmat('E:\Super-BPD-master\post_process/2009_004607.mat')['flux']
flux = sio.loadmat('E:\Super-BPD-master\datasets\GRIP\GRIP_seg/TP_C01_002_copy.mat')['flux']
...
# cv2.imshow('flux0', flux[0,...])
# cv2.imshow('flux1', flux[1,...])
cv2.imshow('root_points', 255*(root_points>0))
cv2.imshow('super_BPDs_before_dilation', label2color(super_BPDs_before_dilation))
cv2.imshow('super_BPDs_after_dilation', label2color(super_BPDs_after_dilation))
cv2.imshow('super_BPDs', label2color(super_BPDs))

cv2.waitKey(0)
cv2.destroyAllWindows()

save_name = save_dir + '\\' + name[:-4] + '.png'
cv2.imwrite(save_name, label2color(super_BPDs))
